...
function getNextPageSlugForRedirect() {
  const normalize = (value) => {
    if (!value) return "";
    return value.startsWith("/69caa7eddb49f57dcd6f6503-preview") ? value : (value.startsWith("/") ? "/69caa7eddb49f57dcd6f6503-preview" + value : "/69caa7eddb49f57dcd6f6503-preview/" + value);
  };
...
async function createOrderViaWallet(confirmationToken, paymentMethodId) {
  ...
  const orderData = {
    pageId: "lmF-D0VNY_a8GvnFJ5LTqf37BjWInBexK6jbuk04V6mj4OzaAI1Ky2WIXXRn3t-y",
    action: "process",
...
const getStates = async (countryIso2Code) => {
  ...
  try {
    if (!countriesStatesCache) {
      const res = await fetch('/69caa7eddb49f57dcd6f6503-preview/scripts/countriesData/countriesStates.json', { cache: 'no-store' });
